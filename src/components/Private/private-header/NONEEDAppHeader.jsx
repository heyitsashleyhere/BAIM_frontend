...
export const AppHeader = () => {
	...
	useEffect(() => {
		...
		tl.current.fromTo(
			[navItemsRef.current, logoutRef.current],
			{
				opacity: 0,
			},
			{
	...
	const dsktpTl = useRef(null);
	useEffect(() => {
		pathname.includes("/profile")
			? (dsktpTl.current = gsap
					.timeline({ delay: 4 })
			...
			: (desktopNavRef.current.style.transform = "none");
		return () => {
			if (dsktpTl.current) {
				dsktpTl.current.kill();
			}
		};
	}, []);
	...
	return (
		<>
			<header ref={desktopNavRef} className="app-header">
					...
					<nav className="navbar">
						<ul className="nav-list">
							...
							<li className="nav-item">
								<h1 className="btn LokaB" onClick={logoutUser}>
									Logout
								</h1>
							</li>
						</ul>
				...
				<div ref={sideNavBgRef} className="burger-menu-inner">
					<ul className="burger-menu-list">
						...
						<li className="burger-menu-list-item" key="mobile-logout">
							<h1 ref={logoutRef} className="btn" onClick={logoutUser}>
								Logout
							</h1>
						</li>
					</ul>
