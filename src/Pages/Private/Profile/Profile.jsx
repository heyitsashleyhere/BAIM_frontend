...
export const Profile = () => {
	const { postCategories, upgrade, setUpgrade, profileData, setProfileData } =
		...
		fetch(`http://localhost:7000/user/${profileName}`, config)
			...
			.then((result) => {
				if (result.errors) {
					...
					if (
						result["beauty"].length == 0 &&
						result["artsCraft"].length == 0 &&
						result["garden"].length == 0 &&
			...
			fetch(`http://localhost:7000/${cat}/author/${profileName}/`, config)
				...
				.then((result) => {
					...
					if (!result.errors) {
						...
						console.log(`profile ${cat} fetch errors :>> `, result.errors);
					}
				})
				.catch((error) =>
